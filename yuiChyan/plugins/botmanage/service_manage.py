...
@on_command('admin_lssv', permission=perm.GROUP_ADMIN, only_to_me=False, shell_like=True)
async def lssv(session: CommandSession):
    ...
    svs = map(lambda sv: (sv, sv.judge_enable(gid)), svs)
...
@on_command('sv_enable', permission=perm.GROUP, only_to_me=False)
async def enable_service(session: CommandSession):
    await switch_service(session, turn_on=True)

@on_command('sv_disable', permission=perm.GROUP, only_to_me=False)
...
async def switch_service(session: CommandSession, turn_on: bool):
    ...
    if session.ctx['message_type'] == 'group':
        ...
        for name in names:
            if name in svs:
                sv = svs[name]
                u_priv = priv.get_user_permission(session.ctx)
                if u_priv >= sv.manage_priv:
                    sv.enable_service(group_id) if turn_on else sv.disable_service(group_id)
                    succ.append(name)
                else:
                    continue
            else:
                notfound.append(util.filter_message(name))
        msg = []
        if succ:
        ...
        for gid in group_ids:
            try:
                gid = int(gid)
                sv.enable_service(gid) if turn_on else sv.disable_service(gid)
                succ.append(gid)
            except:
                try:
