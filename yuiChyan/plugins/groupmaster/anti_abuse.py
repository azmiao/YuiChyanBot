...
async def black_filter(bot, ctx, plugin_manager=None):  # plugin_manager is new feature of nonebot v1.6
    ...
    if ctx['message_type'] == 'group' and yuiChyan.priv.check_block_group(ctx['group_id']) \
       or yuiChyan.priv.check_block_user(ctx['user_id']):
...
async def hb_handler(ctx):
    ...
    if first_msg_seg.type == 'hb':
        ...
        if _check_hbtitle_is_cmd(ctx, title):
            yuiChyan.priv.set_block_group(group_id, timedelta(hours=1))
            yuiChyan.priv.set_block_user(user_id, timedelta(days=30))
            await util.silence(ctx, 7 * 24 * 60 * 60)
            msg_from = f"{ctx['user_id']}@[群:{ctx['group_id']}]"
            yuiChyan.logger.critical(f'Self: {ctx["self_id"]}, Message {ctx["message_id"]} from {msg_from} detected as abuse: {ctx["message"]}')
            ...
            try:
                ...
                yuiChyan.logger.critical(f"已将{user_id}移出群{group_id}")
            except:
                pass
...
async def ban_word(session):
    ...
    # yuiChyan.logger.critical(f'Self: {ctx["self_id"]}, Message {ctx["message_id"]} from {msg_from}: {ctx["message"]}')
    hoshino.priv.set_block_user(user_id, timedelta(minutes=1))
    pic = R.img(f"yui{random.randint(1, 4)}.jpg").cqcode
    await session.send(f"哼，竟然骂我，1分钟内我不会回你消息了\n{pic}", at_sender=True)
