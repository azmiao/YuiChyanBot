...
@sv.on_match('开空调')
async def air_con_on(bot, event):
...
@sv.on_match('关空调')
async def air_con_off(bot, event):
    gid = str(event['group_id'])

    air_con = await check_status(gid, event)
    if air_con is None:
        return
...
@sv.on_match('当前温度')
async def air_con_now(bot, event):
    gid = str(event['group_id'])

    air_con = await check_status(gid, event, need_on=False)
    if air_con is None:
        return
...
async def set_temp(bot, event):
    ...
    _set_temp = await check_range(event, -273, 999999, '只能设置-273-999999°C喔')
...
async def set_wind_rate(bot, event):
    ...
    wind_rate = await check_range(event, 1, 3, '只能设置1/2/3档喔',
                                  {'低': 1, '中': 2, '高': 3})
...
async def set_env_temp(bot, event):
    ...
    env_temp = await check_range(event, -273, 999999, '只能设置-273-999999°C喔')
    if env_temp is None:
        return
...
@sv.on_match(('空调类型',))
async def show_air_con_type(bot, event):
    gid = str(event['group_id'])
...
async def upgrade_air_con(bot, event):
    gid = str(event['group_id'])
...
async def downgrade_air_con(bot, event):
    gid = str(event['group_id'])
