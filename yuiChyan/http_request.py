...
async_session_map: Dict[str, ClientSession] = {}


# 获取缓存的session | if_async: 是否是异步的session | create_if_none: 是否要在不存在的时候创建
def get_session_or_create(name: str, if_async: bool = False, create_if_none: bool = True) -> Session | ClientSession:
    ...
    session_type = "ClientSession" if if_async else "Session"
...
# 手动关闭同步会话
def close_session(name: str, session: Session):
...
# 手动关闭异步会话
async def close_async_session(name: str, session: ClientSession):
...
# 保存session至缓存
def save_session(name: str, session: Session | ClientSession):
    if isinstance(session, Session):
...
def create_async_session(name: str, is_save: bool = False) -> ClientSession:
    if name in session_map or name in async_session_map:
